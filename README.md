...
LinguaFlow 是一套基于 **WebSocket 全双工通信** 的实时同声传译解决方案。系统通过**流式语音识别 (ASR)** 将音频实时转写为文本，经由 **LangChain4j 大语言模型** 完成跨语言翻译，最终通过 WebSocket 推送至客户端，实现端到端延迟低于 200ms 的同传体验。
...
┌──────────┐   Audio PCM    ┌─────────────┐   Text      ┌──────────────┐
│          │ ─────────────→ │             │ ──────────→ │              │
│  Browser │                │  Backend    │            │  LLM (AI)    │
│  Client  │ ←───────────── │  Spring Boot│ ←────────── │  Translation │
│          │   Translation  │  WebSocket  │   Result    │              │
└──────────┘                └─────────────┘              └──────────────┘
                                   │
                              ┌────┴────┐
                              │ 阿里云   │
...
```
Backend/src/main/java/com/javaee/backend/
│
├── config/                          # 配置层
...
│   └── InterpretationHandler.java   #   WebSocket 消息分发处理器
│
